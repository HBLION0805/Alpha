...
The six fixed public headline feeds have direct live read-only access. Separately, the authorized host Robinhood connection exposes five working market-data tools; Alpha's capture and observation CLIs are offline. Numerical driver connectors, fully qualified GLD/IBIT quote paths, complete portfolio enforcement, calibrated outcomes, account access and order execution are not implemented. Local persistence is single-process. The existing hourly Codex heartbeat checks public feeds only; it does not collect Robinhood quotes or execute trades. See docs/HANDOFF.md and docs/status/robinhood-observation.json for the latest evidence; current.json is a dated older build checkpoint.
...
The capture and observation CLIs read local exports and cannot authenticate their source or invoke MCP. They preserve source/request/receipt/recording clocks, exclude interpolation, reuse unchanged budgets with unknown costs, and never authorize replay. Captures, study plans and linked frames are immutable local artifacts; candidate data-quality lessons are not trade outcomes or approved knowledge. Never relabel Robinhood data as Cboe, invent historical fills, or rewrite accepted captures/journals. Screening and prospective observation preparation are implemented. Next obtain eligible-session sequential data using the frozen study and assess complete evidence before a separate paper adapter. Preserve missed windows rather than backdating collection; do not expand the public-feed heartbeat implicitly.
