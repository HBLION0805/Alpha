...
The six fixed public headline feeds have direct live read-only access. Separately, the authorized host Robinhood connection exposes five working market-data tools; Alpha's capture, observation and collection CLIs are offline. Numerical driver connectors, fully qualified GLD/IBIT quote paths, complete portfolio enforcement, calibrated outcomes, account access and order execution are not implemented. Local persistence is single-process. The existing gld-ibit heartbeat was read back as daily 09:00 New York, correcting older hourly wording. Under explicit Owner authority it now shares the bounded quote window, with exact original news fields saved for restoration. See docs/HANDOFF.md and docs/status/robinhood-closeout.json for the latest evidence; current.json is a dated older build checkpoint.
