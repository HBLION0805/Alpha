...
Robinhood's official Trading MCP documents options tools. Host OAuth has now been verified, but Alpha has no verified runtime quote response or source adapter. Its public historical-data description is OHLC, not confirmed historical bid/ask sizes. Assess an authorized read-only capability/schema review before assuming a paid provider is the only data route; authentication alone is not quote entitlement, data quality or execution authority.

The data-readiness command inspects local declarations and can make one anonymous fixed-endpoint GET without reading a body or invoking MCP. Its checked-in example remains disabled. After that delivery, the Owner explicitly authorized installing and enabling only get_option_chains, get_option_instruments, get_option_quotes, get_option_historicals and get_equity_quotes, with the Owner completing official login. The Owner also explicitly accepted the broader official OAuth grant after the agent's consent click was rejected by automatic review. The installed host configuration retains the five tools; login completed with CLI exit 0 and independent o_auth readback on 2026-09-07 UTC (September 6 New York). The current task has no callable Robinhood tools yet; the local refresh proxy failed before initialization with Windows socket error 10050. Reload through desktop MCP settings before actual schema/quote verification; do not repeat completed login or consent. This authorization persists without reconfirmation. Stop before any new-account or paid step. Account/order tool calls and brokerage transactions remain unauthorized. The five-tool client filter does not narrow server-side permissions or enforce GLD/IBIT arguments. Never relabel a Robinhood response as Cboe evidence; preserve existing journals and fingerprints. See [host setup](docs/OPTIONS_ROBINHOOD_CONNECTION_SETUP.md) for current operational authority and [data-readiness delivery](docs/OPTIONS_ROBINHOOD_DATA_READINESS_DELIVERY.md) for the earlier implementation snapshot.
