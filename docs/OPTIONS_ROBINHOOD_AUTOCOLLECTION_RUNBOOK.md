...
[active daily context restoration fields](OPTIONS_MARKET_CONTEXT_HEARTBEAT_RESTORE_V4.json), preserving
...
- Before `2026-09-08T13:30:00.000Z`, retain the armed daily 09:00/09:30 schedule.
  At the normal 09:00 New York wake, execute the daily news, Treasury, BTC and BLS calendar context prompt
  from `docs/OPTIONS_MARKET_CONTEXT_HEARTBEAT_RESTORE_V4.json`. At other pre-window wakes,
...
- At or after `2026-09-08T13:50:00.000Z`, first restore the active daily context fields
  in `docs/OPTIONS_MARKET_CONTEXT_HEARTBEAT_RESTORE_V4.json` using `automation_update`.
...
remains immutable. The active v4 baseline retains the v3 news/Treasury/BTC
workflow and adds one BLS calendar refresh at daily 09:00; context refreshes never run during each
