...
Scan Foundation has closed its offline C3 regular-session contract on this
branch and remains limited to offline-only Owner authorization. Phase 1B has
not started and requires separate Owner approval. The recorded `574a9c2` value
is the implementation source baseline, not
...
Phase 1A supports only explicit completed `REGULAR` sessions. `EXTENDED`
sessions fail closed and are deferred; no premarket or after-hours session
support is part of this milestone.
...
13. **Day15-T3B10 — Forward Collection Pilot**
   ...
   - T3B15-C2 status: committed and pushed as `385aa9b`. It separates mutation from independent read-only durable observation, fixes Git by canonical path and digest, rechecks a structural authority seal, defines atomic empty-store Control `1.0` to `1.1` migration, makes validation receipt/result persistence atomic, closes Node/Python process escapes, and adds one closed non-executable production-shaped runtime. Focused suites pass `40/40`, `4/4`, and `11/11`; complete Alpha validation passes `2463/2463`.
   ...
   - T3B15-MR3 status: committed and pushed as `6746947`; decision `NO_GO_FOR_ONE_EXACT_NETWORK_FREE_REHEARSAL`.
   - T3B15-MR3 scope: confirms C2's Git identity, atomic Validate path, empty legacy migration, tested process guards, query-only Control reads, and non-executable runtime; blocks rehearsal on injected rather than concrete durable observation, an open rather than fully fixed composition root, missing Node/Python digests, transient authority substitution, incomplete guard/Git allowlists, inexact legacy-schema recognition, reachable standalone receipt writes, and missing fresh-process verification over both durable stores.
   - T3B15-C3 status: committed and pushed as `43bcca8`; focused authority tests pass `3/3`, Control passes `42/42`, and complete validation passes `2469/2469`.
   - T3B15-C3 scope: concrete query-only rehearsal observation, fixed non-executable Control/observer/verifier composition, digest-bound Git/Node/Python/guard identity, authorization-bound authority snapshots, exact Control 1.0/1.1 schemas, orphan Validate recovery, exact child/Git admission, and dedicated-process source-rehearsal/Control/backup verification.
   - T3B15-MR4 status: complete locally and pending Owner review; decision `NO_GO_FOR_ONE_EXACT_NETWORK_FREE_REHEARSAL`.
   - T3B15-MR4 scope: confirms exact Control schemas, atomic Validate, orphan detection, query-only durable access, executable/guard digests, and narrowed process admission; blocks rehearsal on caller-injected phase authority, incomplete durable-evidence reconstruction, partial phase-wide authority binding, and no positive fresh-process verification path.
...
15. **Personal MVP-T1 through MVP-T3G — Daily ETF Decision Support**
   - Historical status: T1 through T3G-C13-C2 are committed and pushed through `c9d7cf2`. Current delivery authority is recorded only in `docs/status/current.json`.
   ...
   - T3C: provider coverage registry and readiness gate. Alpaca Basic IEX remains single-venue evidence and is now blocked as the complete provider after `MULS` failed exact P1D live verification.
   ...
   - T3G-C2: complete locally; corrects the invalid two-Bar minimum so one exact completed P1D session is valid, retains exact 12-symbol enforcement, and adds bounded redacted provider-failure diagnostics. Zero-network focused tests pass; another real read requires fresh Owner authorization.
   - T3G-C3: complete locally; preserves the valid `MULS` mapping, records the exact IEX omission without inventing a cause, marks `MULS` failed and all uncompleted checks pending, and deterministically blocks Alpaca Basic IEX for the complete 12-symbol path.
   ...
   - T3G-C13-C1: committed and pushed as `23e2644`; upgrades the changed active catalog to registry version `1.1`, gives it a distinct creation time, removes the two real-MULS package commands, and makes both retained scripts reject before arguments, credentials, or network construction. Historical diagnostic code and fixtures remain audit-only.
   - T3G-C13-C2: committed and pushed as `c9d7cf2`; reconciles the eleven-symbol state without authorizing a live read.
