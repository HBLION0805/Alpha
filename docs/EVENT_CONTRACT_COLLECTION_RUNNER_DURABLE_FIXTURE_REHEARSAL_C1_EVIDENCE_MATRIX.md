...
version-controlled process evidence after `Day15-T3B14-C2`.
...
| 3 | Crash before phase-claim commit | `EventContractCollectionRunnerRuntimeTransactionDrill.test.ts` — `crash before ownership leaves no lock or store mutation` | Child exits before ownership/store mutation; parent inspects the filesystem. |
...
| 5 | Crash after T6 and before rehearsal receipt | `EventContractCollectionRunnerRuntimeTransactionDrill.test.ts` — `crash after T6 preserves DUE without lease or attempt` | Child exits at the injected T6 boundary; parent reopens SQLite read-only. |
...
| 9 | Crash after T10 and before rehearsal receipt | `EventContractCollectionRunnerRuntimeTransactionDrill.test.ts` — `crash after T10 preserves committed evidence and removes lease` | Child exits after atomic evidence commit; parent reopens SQLite read-only. |
...
| 14 | Changed phase, ordinal, recovery, or manifest rejection | `EventContractCollectionRunnerDurableFixtureRehearsalC2ProcessDrill.test.ts` — `fresh-process exact replay is idempotent and changed replay fails closed` | Separate changed-replay child fails closed; fresh inspection proves durable state is unchanged. |
...
| 17 | Package artifact and validation-receipt substitution | `EventContractCollectionRunnerDurableFixtureRehearsalProcessDrill.test.ts` — `validation receipt substitution fails closed`; `EventContractCollectionRunnerFixtureRehearsalPackageProcessDrill.test.ts` — `process crash after durable artifacts leaves no valid package and quarantines partial state` | Fresh verifier rejects substituted validation; killed package child leaves only quarantinable staging. |
...
`T3B14-MR3` must independently review C2 and complete Alpha validation before
