...
function studyCard(r){
  ...
  return `<section class="card section-space event-study"><div class="card-head"><div><p class="eyebrow">FROZEN EXPERIMENT · ${esc(r.plan.id)}</p><h2>${esc(r.plan.title)}</h2><p>${esc(r.plan.event.title)} · ${esc(timestamp(r.plan.event.scheduledAt))}</p></div><button class="button secondary small" data-event-save="${esc(r.plan.id)}">Save evidence snapshot</button></div>
    ...
    <div class="notice"><div>Shared planned-loss allowance: <strong>${money(r.eventRisk.plannedLossBudgetCents)}</strong> · Full-loss stress allowance: <strong>${money(r.eventRisk.fullLossStressBudgetCents)}</strong>. Previous losses consume the allowance. Sale proceeds are not assumed settled.</div></div>
    <div class="table-scroll"><table><thead><tr><th>Comparison</th><th>Assessment</th><th>Net reference</th><th>Loss allowance left</th></tr></thead><tbody>${r.comparisons.map(c=>`<tr><td>${esc(words(c.mode))}</td><td>${status(c.status)}</td><td>${money(c.netReferencePnlCents)}</td><td>${money(c.remainingEventLossCents)}</td></tr>`).join('')}</tbody></table></div>
