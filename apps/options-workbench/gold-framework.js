...
export function goldFrameworkPanel(component) {
  ...
  return '<section class="card section-space" id="gold-framework"><p class="eyebrow">GLD RESEARCH CHECKLIST</p><h2>Gold driver cross-check</h2>'+
    ...
    f.areas.map(a=>'<details class="headline"><summary>'+esc(a.screenshotArea===null?'Additional check':a.screenshotArea+'.')+' '+esc(a.title)+'</summary>'+
      ...
      (a.numericEvidence?.nominal?'<p>'+esc(a.numericEvidence.nominal.description)+' · Source date '+esc(a.numericEvidence.nominal.sourceDate)+' · Received '+esc(timestamp(a.numericEvidence.nominal.receivedAt))+'.</p>':'')+
      (a.numericEvidence?'<p>'+(a.numericEvidence.description?esc(a.numericEvidence.description):'10Y real yield '+esc((a.numericEvidence.tenYearRealYieldBps/100).toFixed(2))+'%')+' · Source date '+esc(a.numericEvidence.sourceDate??'See individual model periods')+' · Received '+esc(timestamp(a.numericEvidence.receivedAt))+'. Current intraday freshness is not established.</p>':'')+
