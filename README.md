...
Day 8 Intelligence foundations through Day15-T3B12-MR1 are complete and pushed. Day13-T2/T3 adds a deterministic BTC 15-minute Event Analyzer console prototype with bounded PT1M candle features, Day14-T1 adds the provider-independent Capital Allocation Framework v1.0 construction boundary, Day15-T1/T2 adds immutable event-contract observations and an append-only shadow ledger, Day15-T3A/T3B adds research-integrity and dataset-qualification gates, and T3B2 through T3B8 build exact offline/forward collection and reviewed fixture/live-read source foundations. Day15-T3B9 defines the runner architecture; T3B10 through T3B12 build and drill the durable SQLite, authenticated recovery, fixture runtime, and one-action assembly foundations while keeping operation blocked. T3B13-T1 through T5 are pushed: they define strict fixture-rehearsal authority, isolated preparation, one explicit exact-ordinal foreground step, bounded evidence packaging, and package-level crash drills. T3B13-MR1 is complete locally and accepts those components as foundations, but blocks a rehearsal run until durable cross-process rehearsal state, concrete SQLite/backup/validation verification, and one end-to-end assembled process drill exist.
