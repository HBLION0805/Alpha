...
Day 8 Intelligence foundations through Day15-T2 are complete and pushed. Day13-T2/T3 adds a deterministic BTC 15-minute Event Analyzer console prototype with bounded PT1M candle features, Day14-T1 adds the provider-independent Capital Allocation Framework v1.0 construction boundary, Day15-T1 adds the first immutable BTC event-contract observation boundary, and Day15-T2 adds manual JSON capture plus an append-only shadow ledger. Day15-T3A locally adds the point-in-time Research Integrity gate required before honest calibration or backtesting; it is pending owner review.
