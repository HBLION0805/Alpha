...
const tests: ReadonlyArray<readonly [string, () => void]> = [
  ["normalizes the official BTC fifteen-minute fixture deterministically", () => {
    ...
    assertEqual(first.status, KalshiEventContractFixtureStatus.NormalizedPendingMapping, "status");
    if (first.status !== KalshiEventContractFixtureStatus.NormalizedPendingMapping
      || second.status !== KalshiEventContractFixtureStatus.NormalizedPendingMapping) return;
  ...
  ["preserves exact official Kalshi identity", () => {
    ...
    if (result.status !== KalshiEventContractFixtureStatus.NormalizedPendingMapping) throw new Error("fixture rejected");
  ...
  ["preserves finalized down settlement without creating an observation", () => {
    ...
    assertEqual(result.eligibleForSourceSnapshot, false, "snapshot eligibility");
    assertEqual(result.sourceSnapshot, null, "snapshot");
  }],
  ["keeps mapping pending despite three matching displayed facts", () => {
    const result = fixture();
    if (result.status !== KalshiEventContractFixtureStatus.NormalizedPendingMapping) throw new Error("fixture rejected");
    assertEqual(result.mappingAssessment.matchingDisplayedFacts.length, 3, "matching facts");
    assertEqual(result.mappingAssessment.eligibleForCollection, false, "mapping eligibility");
    assertTrue(result.mappingAssessment.blockerCodes.includes(KalshiRobinhoodMappingBlockerCode.MissingDeclaredExchangeIdentity), "exchange blocker");
    assertTrue(result.mappingAssessment.blockerCodes.includes(KalshiRobinhoodMappingBlockerCode.MissingPlatformTermsVersion), "terms blocker");
  }],
