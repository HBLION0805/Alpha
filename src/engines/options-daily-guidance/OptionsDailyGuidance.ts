...
export function assessDailyGuidance(input: GuidanceInput) {
  ...
  const sessionWindow=!["Sat","Sun"].includes(local.weekday??"") && local.minute>=570 && local.minute<960;
  ...
  const assets=(["GLD","IBIT"] as const).map(symbol=>{
    ...
    if(Object.hasOwn(input,"analyst")) {
  ...
  return {version:legacyRiskCapsRemoved(settings.tradeBudget)?"OPTIONS_DAILY_GUIDANCE_V3":settings.tradeBudget?"OPTIONS_DAILY_GUIDANCE_V2":"OPTIONS_DAILY_GUIDANCE_V1",assessedAt:input.at,marketCapturedAt:input.captureAt,assets,events:upcoming,sourceHealth:input.sourceHealth,headlines:input.headlines,context:input.context,settings,
