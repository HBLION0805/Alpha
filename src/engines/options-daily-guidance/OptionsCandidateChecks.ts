...
export function assessCandidateChecks(input: CandidateChecksInput) {
  ...
  const rows = original.assets.flatMap(asset => asset.candidates.map(candidate => {
    ...
    add("allocation", "All-in allocation", ["ALLOCATION_BUDGET_EXCEEDED"], `Premium ${usd(e?.premiumCents)}; capital required ${usd(e?.capitalRequiredCents)}; 5% allocation ceiling ${usd(e?.applicableAllocationBudgetCents)}.`, "Require one whole contract and its declared costs to fit the unchanged allocation ceiling.", !e || costsUnknown && !reasons.includes("ALLOCATION_BUDGET_EXCEEDED"));
    ...
    const premiumWithinBudget = e ? e.premiumCents <= e.applicableAllocationBudgetCents : null;
    return {contract:q, dte:candidate.dte, disposition:candidate.disposition, originalBlockers:reasons, premiumWithinBudget, checks, economics:e, plan:candidate.plan,
      nextActions:[...new Set(checks.filter(c => c.status !== "PASS").map(c => c.nextAction))], paperAdapterQualified:false, executionAllowed:false};
  }));
  return {version:"OPTIONS_CANDIDATE_CHECKS_V1", assessedAt:original.assessedAt, capturedAt:original.marketCapturedAt,
    settings, sourcePaths:input.sourcePaths, originalGuidanceFingerprint:paperFingerprint(original), rows,
    counts:{sampled:rows.length, premiumWithinBudget:rows.filter(r => r.premiumWithinBudget === true).length, premiumOverBudget:rows.filter(r => r.premiumWithinBudget === false).length, premiumUnknown:rows.filter(r => r.premiumWithinBudget === null).length, conditionalResearch:rows.filter(r => r.disposition === "CONDITIONAL_RESEARCH").length, blocked:rows.filter(r => r.disposition === "NO_TRADE").length},
    ...
    scope:"Latest bounded guidance sample only. Existing 14–45 DTE policy, budgets and risk limits are unchanged. Historical close-chain activity remains separate.",
