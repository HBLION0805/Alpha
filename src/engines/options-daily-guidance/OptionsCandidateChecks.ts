...
export function assessCandidateChecks(input: CandidateChecksInput) {
  ...
  const rows = original.assets.flatMap(asset => asset.candidates.map(candidate => {
    ...
    add("session", "Regular session window", ["OUTSIDE_REGULAR_SESSION", "SESSION_CALENDAR_YEAR_UNREVIEWED"], input.guidance.version==="OPTIONS_DAILY_GUIDANCE_INPUT_V2"?"Reviewed 2026 regular-session calendar, including holidays and early closes; halts and contract-specific qualification remain unverified.":"Existing New York weekday/time envelope; separate exchange-session qualification is still required.", "Reassess during an eligible regular session with fresh quotes.");
    ...
    add("direction", "Trend and attributed context", ["DIRECTION_NOT_CONFIRMED", "NEWS_AND_TREND_NOT_ALIGNED", "ATTRIBUTED_CONTEXT_REVIEW_MISSING_OR_OLD", "MARKET_EVIDENCE_NEWER_THAN_ANALYSIS", "OPPOSES_OBSERVED_TREND", "HEADLINE_REFRESH_UNAVAILABLE"], `${asset.trend.direction}; ${asset.trend.closes.length} distinct recent closes in the existing trend window.`, "Resolve missing/conflicting trend and source-backed context before considering a direction.", asset.trend.direction === "INSUFFICIENT_HISTORY");
    add("events", "Calendar and event posture", ["CALENDAR_COVERAGE_UNAVAILABLE", "MAJOR_EVENT_WAIT",...(original.eventEntry?.blockers??[]),'EVENT_NOT_IN_CURRENT_GUIDANCE_CALENDAR','EVENT_PLAN_CONTRACT_MISMATCH'], original.eventEntry?[original.eventEntry.explanation,...original.eventEntry.risks].join(' '):"Existing major-event wait windows remain in force.", original.eventEntry?"Review the selected saved plan, listed missing confirmations and current calendar; other evidence checks remain independent.":"Verify calendar coverage and reassess after the existing event wait window.", !input.guidance.calendarAvailable);
  ...
  return {version:removed?"OPTIONS_CANDIDATE_CHECKS_V3":settings.tradeBudget?"OPTIONS_CANDIDATE_CHECKS_V2":"OPTIONS_CANDIDATE_CHECKS_V1", assessedAt:original.assessedAt, capturedAt:original.marketCapturedAt,
    settings, sourcePaths:input.sourcePaths, originalGuidanceFingerprint:paperFingerprint(original), rows,...(original.eventEntry?{eventEntry:original.eventEntry}:{}),
