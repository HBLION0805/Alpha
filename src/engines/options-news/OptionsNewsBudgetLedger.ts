...
export class OptionsNewsBudgetLedger {
  ...
  public reserve(reservationId: string, providerId: string, atUtc: string, estimateMinorUnits: number | null): NewsBudgetReservation {
    const month = atUtc.slice(0,7); const existing = this.reservations.get(reservationId); if (existing !== undefined) { if (existing.providerId === providerId && existing.monthUtc === month && existing.estimatedMinorUnits === estimateMinorUnits) return structuredClone(existing); throw new Error("BUDGET_IDEMPOTENCY_CONFLICT"); }
  ...
  public reconcile(reservationId: string, actualMinorUnits: number): NewsBudgetReservation { const existing = this.required(reservationId); if (!Number.isSafeInteger(actualMinorUnits) || actualMinorUnits < 0 || actualMinorUnits > existing.estimatedMinorUnits) throw new Error("INVALID_ACTUAL_COST"); const value = { ...existing, state: "RECONCILED" as const, actualMinorUnits }; this.reservations.set(reservationId, value); return structuredClone(value); }
  public release(reservationId: string): NewsBudgetReservation { const existing = this.required(reservationId); const value = { ...existing, state: "RELEASED" as const, actualMinorUnits: 0 }; this.reservations.set(reservationId, value); return structuredClone(value); }
  public snapshot(atUtc: string): NewsBudgetSnapshot { const month = atUtc.slice(0,7); const values = [...this.reservations.values()].filter((v) => v.monthUtc === month); const committed = values.filter((v) => v.state === "RECONCILED").reduce((sum,v) => sum + (v.actualMinorUnits ?? 0),0); const reserved = values.filter((v) => v.state === "RESERVED").reduce((sum,v) => sum + v.estimatedMinorUnits,0); const total = committed + reserved; return { monthUtc: month, committedMinorUnits: committed, reservedMinorUnits: reserved, state: total >= this.policy.hardLimitMinorUnits ? NewsBudgetState.BlockedBudget : total >= this.policy.softLimitMinorUnits ? NewsBudgetState.Warning : NewsBudgetState.Normal }; }
