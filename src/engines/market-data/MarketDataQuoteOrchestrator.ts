...
export class MarketDataQuoteOrchestrator {
  public constructor(
  ...
  public async getLatestQuote(value: unknown): Promise<MarketDataResult> {
    ...
    const adapter = this.composition.getQuoteAdapter(request.providerId);
    ...
    if (provider === undefined || !provider.defaultEnabled) {
      return resultWithoutData(
        request,
        startedAt,
        MarketDataResultStatus.Unavailable,
