...
export function assessPaperCollectionPlan(plan:SnapshotPlan,at:string,calendar:PaperCollectionCalendar) {
  ...
  const fixed=guidanceFixedMinutes(date),paper=guidancePaperMinutes(date),minutes=[...new Set([...fixed,...paper,...GUIDANCE_EVENT_MINUTES])].sort((a,b)=>a-b);
  ...
  const pairs=entry.flatMap(a=>exits.filter(b=>b.wakeAt>a.wakeAt).map(b=>({entry:a,exit:b,conditional:a.basis==='EVENT_CONDITIONAL'||b.basis==='EVENT_CONDITIONAL'})));
  ...
  const status=!session.knownYear?'SESSION_UNREVIEWED':session.holiday?'SESSION_CLOSED':now>=start?'ENROLLMENT_WINDOW_PASSED':pair?(pair.conditional?'CONDITIONAL_PAIR':pair.entry.basis==='PAPER_REHEARSAL'||pair.exit.basis==='PAPER_REHEARSAL'?'PAPER_REHEARSAL_PAIR':'ROUTINE_PAIR'):'SCHEDULE_GAP';
