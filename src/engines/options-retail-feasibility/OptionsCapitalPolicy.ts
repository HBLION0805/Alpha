...
export function assessOptionsCapitalPolicy(value:unknown) {
  const settings=settingsInput(value),limits=optionsRetailRiskLimits(settings.currentEquityCents);
  ...
  const riskRoom=BigInt(limits.plannedRiskBudgetCents)-fees-slippage;
  // floor(premium * rate / 10000) <= riskRoom, with capital = premium + fees.
  const plannedCapitalUpperBound=riskRoom<0n?0:Number(fees+((riskRoom+1n)*10000n-1n)/rate);
  ...
  const upperBound=Math.min(maximum,settings.settledCashCents,limits.fullPremiumStressCapCents,plannedCapitalUpperBound);
  ...
  ] as const)if(minimum>limit)conflicts.push({code,requiredCents:minimum,limitCents:limit,message});
  ...
  const rows=levels.map(capitalCents=>{
    ...
    return {capitalCents,premiumCents:settings.roundTripFeesCents!==null&&valid?Number(premium):null,
      ...
      plannedLossExceedsCap:lowerRisk===null?null:lowerRisk>limits.plannedRiskBudgetCents,
      stressExceedsCap:capitalCents>limits.fullPremiumStressCapCents,
  ...
  return freeze({version:"OPTIONS_CAPITAL_POLICY_V1" as const,settings,limits,
