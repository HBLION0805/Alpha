...
import { reportTreasuryHistory, treasuryFingerprint, treasuryUrl } from "../src/engines/options-treasury/TreasuryRealYieldEngine.ts";

const root = resolve(import.meta.dirname, ".."), xml = readFileSync(join(root, "fixtures/options-treasury/real-yields.synthetic.xml"), "utf8");
const at = "2026-09-07T03:00:00.000Z", later = "2026-09-07T04:00:00.000Z";
const input = (sourceText = xml, time = at) => ({ requestedAt: time, receivedAt: time, url: treasuryUrl(time), sourceText, errorCode: null });
const response = (body = xml, extra = {}) => new Response(body, { headers: { "content-type": "text/xml; charset=UTF-8" }, ...extra });
const retrieve = fetchImplementation => retrieveTreasury({ fetchImplementation, clock: () => at });
const journal = (directory, operation, time = later) => withTreasuryJournal(directory, operation, time);
async function temporary(work) {
  const directory = mkdtempSync(join(tmpdir(), "alpha-treasury-test-"));
  try { await work(directory); } finally {
    const rel = relative(tmpdir(), directory);
    if (isAbsolute(rel) || rel.startsWith("..") || !rel.startsWith("alpha-treasury-test-")) throw Error("UNSAFE_TEST_CLEANUP");
...
function rewrite(path, change, rehash = false) {
  ...
  change(records);
  if (rehash) { let previous = null; for (const record of records) { record.previousFingerprint = previous; const { fingerprint, ...body } = record; record.fingerprint = treasuryFingerprint(body); previous = record.fingerprint; } }
  writeFileSync(path, records.map(r => JSON.stringify(r)).join("\n") + "\n");
}
let passed = 0;
async function test(name, work) { await work(); passed++; console.log(`PASS ${name}`); }
await test("one fixed current-month request omits credentials and forbids redirects", async () => {
  let count = 0; const r = await retrieve(async (url, options) => { count++; assert.equal(url, treasuryUrl(at)); assert.equal(options.credentials, "omit"); assert.equal(options.redirect, "manual"); assert(!Object.keys(options.headers).some(k => /authorization|cookie/i.test(k))); return response(); });
  assert.equal(count, 1); assert.equal(r.assessment.status, "OK"); assert.equal(r.input.sourceText, xml);
});
await test("request and receipt preserve separate actual clock samples", async () => {
  let ticks = 0; const r = await retrieveTreasury({ clock: () => ticks++ ? later : at, fetchImplementation: async () => response() });
...
await test("request scope is validated before HTTP starts", async () => {
  let calls = 0; await assert.rejects(retrieveTreasury({ clock: () => "bad", fetchImplementation: async () => { calls++; return response(); } }), /CLOCK/); assert.equal(calls, 0);
});
...
await test("mismatched types and declared size overflow reject", async () => {
  for (const [headers, code] of [[{ "content-type": "text/html" }, "CONTENT_TYPE"], [{ "content-type": "application/xml; charset=latin1" }, "CONTENT_TYPE"], [{ "content-type": "text/xml", "content-length": "524289" }, "BODY_TOO_LARGE"]]) assert.equal((await retrieve(async () => response("private", { headers }))).input.errorCode, code);
});
...
await test("empty body and empty valid XML are distinct", async () => { assert.equal((await retrieve(async () => response(""))).input.errorCode, "BODY_MISSING"); const r = await retrieve(async () => response(xml.replace(/<entry>[\s\S]*?<\/entry>/g, ""))); assert.equal(r.assessment.status, "EMPTY"); assert.equal(r.input.errorCode, null); });
...
await test("a stalled HTTP operation is cut off by a bounded deadline", async () => { const r = await retrieveTreasury({ fetchImplementation: () => new Promise(() => {}), clock: () => at, deadlineMs: 5 }); assert.equal(r.input.errorCode, "DEADLINE_EXCEEDED"); });
await test("a stalled body is bounded by the same request deadline", async () => { const r = await retrieveTreasury({ fetchImplementation: async () => ({ status: 200, headers: new Headers({ "content-type": "text/xml" }), body: { getReader: () => ({ read: () => new Promise(() => {}), cancel: async () => {} }) } }), clock: () => at, deadlineMs: 5 }); assert.equal(r.input.errorCode, "DEADLINE_EXCEEDED"); });
await test("clock rollback fails rather than recording invented receipt time", async () => { let i = 0; await assert.rejects(retrieveTreasury({ fetchImplementation: async () => response(), clock: () => i++ ? at : later }), /CLOCK_ROLLBACK/); });
await test("journal stores exact source, assessment and a linked restartable history", () => temporary(async directory => {
  ...
  await journal(directory, s => { assert.equal(s.inputs[0].sourceText, xml); assert.equal(s.previousFingerprint, first.fingerprint); s.append(input(xml.replace("1.90", "1.91"), later), later); });
  await journal(directory, s => { assert.equal(s.inputs.length, 2); assert.equal(reportTreasuryHistory(s.inputs, later).versions.length, 3); });
...
await test("failed source attempts survive recovery but cannot carry raw errors", () => temporary(async directory => {
  ...
  await journal(directory, s => { assert.equal(s.inputs[0].sourceText, null); assert.equal(reportTreasuryHistory(s.inputs, later).latestRetrieval.status, "FAILED"); });
}));
...
await test("rehashed stored assessments must still match recomputed XML", () => temporary(async directory => {
  let path; await journal(directory, s => { path = s.path; s.append(input(), at); }); rewrite(path, rs => { rs[0].assessment.rows[0].ratesBps[10] = 999; }, true);
...
await test("stale or overlapping retrieval clocks cannot be appended", () => temporary(async directory => {
  await journal(directory, s => { s.append(input(xml, later), later); assert.throws(() => s.append(input(), later), /HISTORY_CLOCK/); }); await journal(directory, s => assert.equal(s.inputs.length, 1));
}));
await test("recovery rejects stored observations from the future", () => temporary(async directory => {
  await journal(directory, s => s.append(input(xml, later), later)); await assert.rejects(journal(directory, () => {}, at), /HISTORY_CLOCK/);
}));
await test("A to B to A revisions and first sightings survive restarts", () => temporary(async directory => {
  ...
  await journal(directory, s => s.append(input(), at)); await journal(directory, s => s.append(input(xml.replace("1.90", "1.91"), later), later));
  await journal(directory, s => s.append(input(xml, third), third), third);
  await journal(directory, s => { const r = reportTreasuryHistory(s.inputs, third); assert.equal(r.versions.length, 4); assert.equal(r.versions[3].firstObservedAt, third); }, third);
}));
...
await test("caller mutation does not change staged history used by append", () => temporary(async directory => {
  await journal(directory, s => { const original = input(); s.append(original, at); original.sourceText = "corrupt"; s.inputs.push(input("corrupt")); s.append(input(xml, later), later); });
  await journal(directory, s => { assert.equal(s.inputs.length, 2); assert.equal(s.inputs[0].sourceText, xml); });
}));
await test("CLI rejects arbitrary URLs, dates, roots and trading modes before any retrieval", () => {
  for (const args of [["--url", "https://example.com"], ["--refresh", "--date", "2026-09-01"], ["--root", "other"], ["--trade"]]) {
    const r = spawnSync(process.execPath, [join(root, "node_modules/tsx/dist/cli.mjs"), join(root, "scripts/options-treasury.mjs"), ...args], { encoding: "utf8", timeout: 30000 });
    assert.equal(r.status, 2); assert.equal(JSON.parse(r.stderr.trim()).code, "TREASURY_ARGUMENTS");
  }
});
