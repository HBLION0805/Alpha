...
import { runOptionsStructuresCommand as run } from "./options-structures.mjs";
import { structureComparisonFixture } from "../src/engines/options-structure-comparison/OptionsStructureComparisonFixtures.ts";
import { readinessFingerprint } from "../src/engines/options-readiness/OptionsReadinessEngine.ts";
const root = resolve(import.meta.dirname, ".."), at = "2026-09-07T19:00:00.000Z", base = "data/runtime/options-structure-comparison";
const options = dir => ({ workspaceRoot: dir, now: () => at });
function temporary(work) { const dir = mkdtempSync(join(tmpdir(), "alpha-structures-test-")); try { work(dir); } finally { const rel = relative(realpathSync(tmpdir()), realpathSync(dir)); if (isAbsolute(rel) || rel.startsWith("..") || !rel.startsWith("alpha-structures-test-")) throw Error("UNSAFE_TEST_CLEANUP"); rmSync(dir, { recursive: true }); } }
function input(dir, data = structureComparisonFixture()) { const bytes = Buffer.from(JSON.stringify(data, null, 2) + "\n"); writeFileSync(join(dir, "input.json"), bytes); return bytes; }
...
test("read-only demo retains all alternatives without creating directories or journals", () => temporary(dir => {
  const r = run(["--demo"], options(dir)); assert.equal(r.cases.length, 2); assert.equal(r.cases[0].result.calculatedCount, 9); assert.equal(r.cases[0].result.uncalculatedCount, 1); assert.deepEqual(readdirSync(dir), []); assert.equal(r.executionAllowed, false);
}));
test("exclusive demo save and verification retain exact bytes and clocks", () => temporary(dir => {
  ...
  assert.equal(r.status, "STRUCTURE_ARTIFACT_SAVED_AND_RECOMPUTED"); assert.equal(v.artifactSha256, r.artifactSha256); assert.equal(v.recordedAt, at); assert.equal(v.calculatedCandidates, 18); assert.equal(v.uncalculatedCandidates, 2); assert.deepEqual(readFileSync(file(dir)), before);
}));
test("saved local input keeps original input hash and source bytes", () => temporary(dir => {
  ...
  assert.equal(saved.source.kind, "LOCAL_FILE"); assert.equal(saved.source.sha256, createHash("sha256").update(bytes).digest("hex")); assert.deepEqual(readFileSync(join(dir, "input.json")), bytes); assert.equal(saved.cases.length, 1); assert.equal(saved.cases[0].input.asOf, "2026-09-08T14:00:00.000Z");
}));
test("a separate process can recompute the saved comparison without its source input file", () => temporary(dir => {
  ...
  const code = `import {runOptionsStructuresCommand as run} from ${JSON.stringify(pathToFileURL(join(root, "scripts/options-structures.mjs")).href)}; console.log(JSON.stringify(run(['--verify','test-artifact'],{workspaceRoot:${JSON.stringify(dir)},now:()=>${JSON.stringify(at)}})));`;
  const child = spawnSync(process.execPath, ["--import", "tsx", "--input-type=module", "-e", code], { cwd: root, encoding: "utf8" }); assert.equal(child.status, 0, child.stderr); assert.equal(JSON.parse(child.stdout).status, "STRUCTURE_ARTIFACT_RECOMPUTED");
}));
...
test("changed results fail recomputation even if the envelope was rehashed", () => temporary(dir => {
  save(dir); const saved = JSON.parse(readFileSync(file(dir), "utf8")); saved.cases[0].result.candidates[0].terminal.maximumLossCents = 0;
...
test("synthetic-demo provenance cannot be relabeled onto a changed declaration", () => temporary(dir => {
  save(dir); const saved = JSON.parse(readFileSync(file(dir), "utf8")); saved.cases[0].input.comparisonId = "different-demo"; writeFileSync(file(dir), JSON.stringify(saved, null, 2) + "\n");
  assert.throws(() => run(["--verify", "test-artifact"], options(dir)), /DEMO_INPUT_MISMATCH/);
}));
test("duplicate decoded input keys are rejected before economic use", () => temporary(dir => {
  const text = input(dir).toString().replace('"initialEquityCents": 100000', '"initialEquityCents": 1, "initialEquityC\\u0065nts": 100000'); writeFileSync(join(dir, "input.json"), text);
...
test("help and invalid command paths cannot create data or expose raw input errors", () => temporary(dir => {
  ...
  const child = spawnSync(process.execPath, ["node_modules/tsx/dist/cli.mjs", "scripts/options-structures.mjs", "--refresh", "PRIVATE_BODY"], { cwd: root, encoding: "utf8" }); assert.equal(child.status, 2); assert(!child.stderr.includes("PRIVATE_BODY")); assert.equal(JSON.parse(child.stderr).executionAllowed, false); assert.deepEqual(readdirSync(dir), []);
}));
