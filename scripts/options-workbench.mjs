...
const assets=new Map([['/',['index.html','text/html']],['/index.html',['index.html','text/html']],...['app.js','api.js','model.js','views.js','forms.js','guidance.js','candidate-checks.js','cost-desk.js','capital-policy.js','gold-framework.js','focused-news.js','event-research.js','bar-quality.js'].map(f=>['/'+f,[f,'text/javascript']]),['/styles.css',['styles.css','text/css']],['/icon.svg',['icon.svg','image/svg+xml']]]);
...
export async function startOptionsWorkbench({port=4173,refreshContext=false,...options}={}){
  ...
  const server=createServer(async(req,res)=>{
    ...
    try{
      ...
      if(req.method!=='POST'||!['/api/preview','/api/save','/api/evaluate','/api/initialize','/api/guidance-settings','/api/event-research','/api/candidate-checks','/api/cost-desk','/api/capital-policy'].includes(url.pathname)||url.search)return reject(404,'ROUTE_NOT_FOUND');
