...
export async function runOptionsContextCutoffCommand(args, { workspaceRoot = process.cwd(), now = () => new Date().toISOString() } = {}) {
  if (args.length === 1 && args[0] === "--help") return { usage: "options:context-cutoff -- --at <canonical-UTC-time>", meaning: "Reconstruct context from local journal receipt/discovery times. No network, source writes, trade replay or claim that a historical decision used this data.", executionAllowed: false };
  if (args.length !== 2 || args[0] !== "--at") throw Error("CONTEXT_CUTOFF_ARGUMENTS");
  ...
  return reconstructOptionsContext(histories, cutoff, now());
}
