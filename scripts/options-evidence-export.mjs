...
import { createExportManifest, validateExportManifest, exportSourcePolicy, exportFingerprint, exportId, exportClock, EXPORT_JOURNALS, EXPORT_MAX_BYTES, EXPORT_MAX_FILES } from "../src/engines/options-evidence-export/OptionsEvidenceExportEngine.ts";
...
function sourceInventory(root, studyId) {
  const paths = [];
  for (const source of Object.values(EXPORT_JOURNALS)) {
  ...
  for (const [kind, limit] of [["studies", 121], ["collection-attempts", 120], ["collection-reports", 120]]) {
    ...
    for (const name of list(root, rel, limit)) { const path = rel + "/" + name; exportSourcePolicy(studyId, path); paths.push(path); }
  }
  ...
  return paths.sort().map((sourcePath, index) => {
    const policy = exportSourcePolicy(studyId, sourcePath), bytes = readBytes(root, sourcePath, policy.maxBytes);
...
function verify(root, packageId, now) {
  ...
  for (const file of manifest.files) {
    const bytes = readBytes(root, base + "/" + file.payloadName, exportSourcePolicy(manifest.studyId, file.sourcePath).maxBytes);
...
export function runOptionsEvidenceExportCommand(args, { workspaceRoot = process.cwd(), now = () => new Date().toISOString() } = {}) {
  if (args.length === 1 && args[0] === "--help") return { usage: ["options:evidence-export -- --create <study-id> <package-id>", "options:evidence-export -- --verify <package-id>"],
  ...
  const before = sourceInventory(root, studyId);
  directory(root, BASE); mkdirSync(safePath(root, base));
  for (const file of before) {
    const bytes = readBytes(root, file.sourcePath, exportSourcePolicy(studyId, file.sourcePath).maxBytes);
  ...
  const after = sourceInventory(root, studyId);
  if (exportFingerprint(before) !== exportFingerprint(after)) fail("SOURCE_CHANGED");
  const completedAt = now(); exportClock(completedAt);
  const manifest = createExportManifest(packageId, studyId, startedAt, completedAt, before), encoded = Buffer.from(JSON.stringify(manifest, null, 2) + "\n");
