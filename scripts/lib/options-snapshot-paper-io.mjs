...
import {snapshotCents,snapshotNs,assessSnapshotQuote,validateSnapshotPlan,replaySnapshotPaper,SOURCE_GAPS} from '../../src/engines/options-robinhood-data/RobinhoodSnapshotPaper.ts';
...
export function mapSnapshotSource(copy){
  ...
  const instruments=new Map(),rawQuotes=new Map();
  ...
  const quotes=normalized.quotes.map(q=>{
    ...
    return {...q,bidCents:snapshotCents(raw.bid_price),askCents:snapshotCents(raw.ask_price),underlyingPriceCents:snapshotCents(eq?.price),underlyingAt:eq?.sourceAt??null,
...
function prepare(root,request,at){
  if(!request||Object.keys(request).sort().join()!==[...REQUEST_FIELDS].sort().join())fail('REQUEST_FIELDS');
  id(request.id);snapshotNs(at);
  const copies=snapshotSources(root),selection=copies.find(c=>c.path===request.selectionPath);
  if(!selection)fail('SELECTION_MISSING');const f=mapSnapshotSource(selection),contract=f.quotes.find(q=>q.id===request.contractId);
  ...
  const {contractId,settingsFingerprint,...rest}=request;
  const plan=validateSnapshotPlan({...rest,version:'OPTIONS_SNAPSHOT_PAPER_PLAN_V1',createdAt:at,contract,settings});
  ...
  const report=replaySnapshotPaper(plan,sources.map(mapSnapshotSource),at);
...
function validateRegistration(r){
  ...
  if(paperFingerprint({...rest,contractId:contract.id,settingsFingerprint:paperFingerprint(settings)})!==paperFingerprint(r.request)||r.selection.path!==p.selectionPath)fail('REGISTRATION_LINKAGE');
  replaySnapshotPaper(p,[mapSnapshotSource(r.selection)],p.createdAt);return r;
}
...
export function saveSnapshotPaperReport(root,planId,at=new Date().toISOString()){
  ...
  const report=replaySnapshotPaper(registration.plan,sources.map(mapSnapshotSource),at),dir=BASE+'/reports/'+id(planId);
...
export function verifySnapshotPaper(root,path){
  ...
  if(r.version==='OPTIONS_SNAPSHOT_PAPER_REGISTRATION_V1'){
    ...
    const report=replaySnapshotPaper(r.registration.plan,r.sources.map(mapSnapshotSource),r.recordedAt);
...
export function snapshotPaperView(root,at=new Date().toISOString()){
  const sources=snapshotSources(root),frames=sources.map(mapSnapshotSource).filter(f=>snapshotNs(f.recordedAt)<=snapshotNs(at)).sort((a,b)=>a.recordedAt.localeCompare(b.recordedAt)),latest=frames.at(-1);
  const cases=files(root,BASE+'/plans').map(path=>{const r=validateRegistration(read(root,path)),reports=files(root,BASE+'/reports/'+r.plan.id).map(path=>{verifySnapshotPaper(root,path);const v=read(root,path);return {path,recordedAt:v.recordedAt,report:v.report};});return {path,plan:r.plan,current:replaySnapshotPaper(r.plan,frames.filter(f=>f.origin===mapSnapshotSource(r.selection).origin&&f.quotes.some(q=>q.id===r.plan.contract.id)),at),snapshots:reports};});
  return {version:'OPTIONS_SNAPSHOT_PAPER_DESK_V1',assessedAt:at,settingsFingerprint:paperFingerprint(guidanceSettings(root)),gates:{quotes:'OPEN',adapter:'LOCAL_IMPLEMENTED_UNQUALIFIED',completeRealPriceLifecycle:'OPEN'},sourceGaps:[...SOURCE_GAPS],latest:latest?{path:latest.path,sha256:latest.sha256,recordedAt:latest.recordedAt,capturedAt:latest.capturedAt,origin:latest.origin,requestedQuoteCount:latest.requestedQuoteCount,missingQuoteIds:latest.missingQuoteIds,quotes:latest.quotes.map(q=>({contract:q,...assessSnapshotQuote(q,at)}))}:null,cases,actualTrades:0,executionAllowed:false};
}
