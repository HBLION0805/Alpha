...
import {snapshotCents,snapshotNs,snapshotUnderlyingMicros,modernSnapshotPlan,assessSnapshotQuote,validateSnapshotPlan,replaySnapshotPaper,PAPER_V2_GAPS,PAPER_FEE_PROFILE} from '../../src/engines/options-robinhood-data/RobinhoodSnapshotPaper.ts';
...
const sourceMode=p=>p.version==='OPTIONS_SNAPSHOT_PAPER_PLAN_V3'?'V3':modernSnapshotPlan(p.version);
export function mapSnapshotSource(copy,v2=false){
  ...
  const quotes=normalized.quotes.map(q=>{
    ...
    return {...q,...(v2===true||v2==='V3'?{chainSession}:{}),...(v2==='V3'?{underlyingPriceUsd:eq?.price??null}:{}),bidCents:snapshotCents(raw.bid_price),askCents:snapshotCents(raw.ask_price),underlyingPriceCents:snapshotCents(eq?.price),underlyingAt:eq?.sourceAt??null,
...
function prepare(root,request,at){
  const v3=request?.modelVersion==='V3',v2=v3||request?.modelVersion==='V2',mode=v3?'V3':v2;
  ...
  if(!selection)fail('SELECTION_MISSING');const f=mapSnapshotSource(selection,mode),contract=f.quotes.find(q=>q.id===request.contractId);
  ...
  const plan=validateSnapshotPlan({...rest,version:v3?'OPTIONS_SNAPSHOT_PAPER_PLAN_V3':v2?'OPTIONS_SNAPSHOT_PAPER_PLAN_V2':'OPTIONS_SNAPSHOT_PAPER_PLAN_V1',createdAt:at,contract,settings});
  ...
  const report=replaySnapshotPaper(plan,sources.map(c=>mapSnapshotSource(c,mode)),at);
...
function validateRegistration(r){
  ...
  if(hasEvent&&!modernSnapshotPlan(r.plan.version)||!hasEvent&&(Object.hasOwn(r,'eventContext')||Object.hasOwn(r.request,'eventPlan')))fail('REGISTRATION');
  const p=validateSnapshotPlan(r.plan),{version,createdAt,contract,settings,...rest}=p;
  if(paperFingerprint({...rest,...(modernSnapshotPlan(version)?{modelVersion:version==='OPTIONS_SNAPSHOT_PAPER_PLAN_V3'?'V3':'V2'}:{}),...(hasEvent?{eventPlan:r.eventContext?.declaration}:{}),contractId:contract.id,settingsFingerprint:paperFingerprint(settings)})!==paperFingerprint(r.request)||r.selection.path!==p.selectionPath)fail('REGISTRATION_LINKAGE');
  eventContext(r);
  replaySnapshotPaper(p,[mapSnapshotSource(r.selection,sourceMode(p))],p.createdAt);return r;
}
...
export function saveSnapshotPaperReport(root,planId,at=new Date().toISOString()){
  ...
  const report=contextualReport(registration,replaySnapshotPaper(registration.plan,sources.map(c=>mapSnapshotSource(c,sourceMode(registration.plan))),at)),dir=BASE+'/reports/'+id(planId);
...
export function saveSnapshotObservation(root,planId,trigger,at=new Date().toISOString()){
  const registration=validateRegistration(read(root,BASE+'/plans/'+id(planId)+'.json'));
  if(!modernSnapshotPlan(registration.plan.version))fail('OBSERVATION_PLAN_VERSION');
  ...
  if(!sources.some(c=>c.sha256===registration.selection.sha256&&c.path===registration.selection.path))fail('SELECTION_CHANGED');
  const report=contextualReport(registration,replaySnapshotPaper(registration.plan,sources.map(c=>mapSnapshotSource(c,sourceMode(registration.plan))),at));
  if(files(root,dir).length>=100)fail('CATALOG');
...
export function verifySnapshotPaper(root,path){
  ...
  if(['OPTIONS_SNAPSHOT_PAPER_REGISTRATION_V1','OPTIONS_SNAPSHOT_PAPER_REGISTRATION_V2'].includes(r.version)){
    ...
    if(r.version==='OPTIONS_SNAPSHOT_PAPER_OBSERVATION_V1'){
      const o=r.observation,p=r.registration.plan;if(!modernSnapshotPlan(p.version)||!o||!['CAPTURE','WINDOW_END'].includes(o.kind))fail('OBSERVATION_TRIGGER');
    ...
    const report=contextualReport(r.registration,replaySnapshotPaper(r.registration.plan,r.sources.map(c=>mapSnapshotSource(c,sourceMode(r.registration.plan))),r.recordedAt));
...
/** Read-only explanation; never appended to original report fingerprints. */
export function snapshotUnderlyingEvidence(frames,contractId,at){
  return frames.flatMap(f=>f.quotes.filter(q=>q.id===contractId).map(q=>{
    const exactPriceValid=snapshotUnderlyingMicros(q.underlyingPriceUsd)!==null;
    const legacyPriceSupported=Number.isSafeInteger(q.underlyingPriceCents)&&q.underlyingPriceCents>0&&q.underlyingPriceCents<=100000000;
    const clockBlockers=assessSnapshotQuote(q,at,true,true).blockers.filter(b=>b.startsWith('UNDERLYING_')&&b!=='UNDERLYING_PRICE_INVALID'&&b!=='UNDERLYING_OUTSIDE_MODEL_SESSION');
    return {sourcePath:f.path,underlyingPriceUsd:q.underlyingPriceUsd,underlyingAt:q.underlyingAt,receivedAt:q.receivedAt,exactPriceValid,legacyPriceSupported,clockBlockers,
      explanation:exactPriceValid&&!legacyPriceSupported?'The source ETF price includes fractions of a cent. The legacy adapter maps it to null; its generic UNDERLYING_UNALIGNED code is not proof of a timestamp failure. V3 preserves the exact price.':'Underlying price precision is supported or unavailable; inspect the independent clock checks.'};
  })).slice(-8);
}
export function snapshotPaperView(root,at=new Date().toISOString()){
  ...
  const sources=snapshotSources(root),frames=sources.map(c=>mapSnapshotSource(c,'V3')).filter(f=>snapshotNs(f.recordedAt)<=snapshotNs(at)).sort((a,b)=>a.recordedAt.localeCompare(b.recordedAt)),latest=frames.at(-1);
  const cases=files(root,BASE+'/plans').map(path=>{const r=validateRegistration(read(root,path)),reports=files(root,BASE+'/reports/'+r.plan.id).map(path=>{verifySnapshotPaper(root,path);const v=read(root,path);return {path,recordedAt:v.recordedAt,report:v.report,...(v.observation?{observation:v.observation}:{})};}).sort((a,b)=>a.recordedAt.localeCompare(b.recordedAt)||a.path.localeCompare(b.path));return {path,plan:r.plan,underlyingEvidence:snapshotUnderlyingEvidence(frames,r.plan.contract.id,at),current:contextualReport(r,replaySnapshotPaper(r.plan,sources.map(c=>mapSnapshotSource(c,sourceMode(r.plan))).filter(f=>f.origin===mapSnapshotSource(r.selection).origin&&f.quotes.some(q=>q.id===r.plan.contract.id)),at)),snapshots:reports};});
  return {version:'OPTIONS_SNAPSHOT_PAPER_DESK_V3',eventPlanning,paperModelImplemented:true,feeProfile:PAPER_FEE_PROFILE,assessedAt:at,settingsFingerprint:paperFingerprint(guidanceSettings(root)),gates:{quotes:'OPEN',adapter:'LOCAL_IMPLEMENTED_UNQUALIFIED',completeRealPriceLifecycle:'OPEN'},sourceGaps:[...PAPER_V2_GAPS],latest:latest?{path:latest.path,sha256:latest.sha256,recordedAt:latest.recordedAt,capturedAt:latest.capturedAt,origin:latest.origin,requestedQuoteCount:latest.requestedQuoteCount,missingQuoteIds:latest.missingQuoteIds,quotes:latest.quotes.map(q=>({contract:q,...assessSnapshotQuote(q,at,true,true)}))}:null,cases,actualTrades:0,executionAllowed:false};
}
