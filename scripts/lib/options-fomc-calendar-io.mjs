...
import { assessFomcCalendar, reportFomcCalendar, fomcCalendarClock, fomcCalendarFingerprint, fomcCalendarUrl, FOMC_CALENDAR_MAX_BYTES } from "../../src/engines/options-fomc-calendar/FomcCalendarEngine.ts";

const MAX_JOURNAL_BYTES = 32 * 1024 * 1024;
const MAX_RETRIEVALS = 366;
const fail = code => { throw Error("FOMC_CALENDAR_" + code); };
/** Exactly one documented, anonymous Federal Reserve date-calendar GET. No provider body enters an error. */
export async function retrieveFomcCalendar({ fetchImplementation = globalThis.fetch, clock = () => new Date().toISOString(), deadlineMs = 12000 } = {}) {
  if (!Number.isSafeInteger(deadlineMs) || deadlineMs < 1 || deadlineMs > 12000) fail("DEADLINE_CONFIGURATION");
  const requestedAt = clock(), url = fomcCalendarUrl(requestedAt), controller = new AbortController();
  ...
  try {
    const response = await withinDeadline(fetchImplementation(url, { redirect: "manual", credentials: "omit", signal: controller.signal,
      headers: { "User-Agent": "Alpha-Options-Research/0.2 (public FOMC meeting calendar)", Accept: "text/html" } }));
    if (response.status !== 200 || response.redirected) throw Error("HTTP_STATUS");
    if (!/^text\/html(?:\s*;\s*charset\s*=\s*(?:utf-8|"utf-8"))?\s*$/i.test(response.headers.get("content-type") ?? "")) throw Error("CONTENT_TYPE");
    const length = response.headers.get("content-length");
    if (length !== null && (!/^\d+$/.test(length) || Number(length) > FOMC_CALENDAR_MAX_BYTES)) throw Error("BODY_TOO_LARGE");
    ...
    while (true) {
      ...
      if (bytes > FOMC_CALENDAR_MAX_BYTES) throw Error("BODY_TOO_LARGE");
  ...
  if (sourceText !== null) {
    try { assessFomcCalendar(input); }
    catch { input = { ...input, sourceText: null, errorCode: "SOURCE_SCHEMA" }; }
  }
  return { input, assessment: assessFomcCalendar(input) };
}
...
export async function withFomcCalendarJournal(workspace, operation, assessedAt = new Date().toISOString()) {
  fomcCalendarClock(assessedAt);
  const root = realpathSync(workspace);
  let directory = root;
  for (const part of ["data", "runtime", "options-fomc-calendar"]) {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (Object.keys(body).sort().join() !== "assessment,input,previousFingerprint,version" || body.version !== "1.0" ||
        body.previousFingerprint !== previousFingerprint || fomcCalendarFingerprint(body) !== fingerprint) fail("JOURNAL_INTEGRITY");
      const assessment = assessFomcCalendar(body.input);
      if (fomcCalendarFingerprint(assessment) !== fomcCalendarFingerprint(body.assessment)) fail("JOURNAL_ASSESSMENT");
      inputs.push(body.input); previousFingerprint = fingerprint;
    }
    reportFomcCalendar(inputs, assessedAt);
    const append = (input, savedAt = new Date().toISOString()) => {
      ...
      const next = structuredClone(input), assessment = assessFomcCalendar(next);
      reportFomcCalendar([...inputs, next], savedAt);
      if (inputs.length >= MAX_RETRIEVALS) fail("JOURNAL_LIMIT");
      const body = { version: "1.0", previousFingerprint, input: next, assessment };
      const fingerprint = fomcCalendarFingerprint(body), line = JSON.stringify({ ...body, fingerprint }) + "\n";
