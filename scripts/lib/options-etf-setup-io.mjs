...
export function verifyEtfSetup(root,path){
  if(!new RegExp('^'+BASE+'/(plans|bars|reports)/[a-zA-Z0-9.-]+\\.json$').test(path))fail('RECORD_PATH');
...
export function etfSetupView(root,guidance,at){
  ...
  const plans=records(root,'plans',at),bars=records(root,'bars',at),reports=records(root,'reports',at);
  return {version:'OPTIONS_ETF_SETUP_DESK_V1',assessedAt:at,
    sourceCapability:'Robinhood documents ETF OHLCV. The local historical-data adapter and a real source response still require verification; file imports are unverified research evidence.',
