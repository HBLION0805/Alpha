...
import { assessReleaseCalendar, reportReleaseCalendar, releaseCalendarClock, releaseCalendarFingerprint, releaseCalendarUrl, RELEASE_CALENDAR_MAX_BYTES } from "../../src/engines/options-release-calendar/BlsReleaseCalendarEngine.ts";

const MAX_JOURNAL_BYTES = 32 * 1024 * 1024;
const MAX_RETRIEVALS = 366;
const fail = code => { throw Error("RELEASE_CALENDAR_" + code); };
/** Exactly one documented, anonymous BLS release-calendar GET. No provider body enters an error. */
export async function retrieveReleaseCalendar({ fetchImplementation = globalThis.fetch, clock = () => new Date().toISOString(), deadlineMs = 12000 } = {}) {
  if (!Number.isSafeInteger(deadlineMs) || deadlineMs < 1 || deadlineMs > 12000) fail("DEADLINE_CONFIGURATION");
  const requestedAt = clock(), url = releaseCalendarUrl(requestedAt), controller = new AbortController();
  ...
  try {
    const response = await withinDeadline(fetchImplementation(url, { redirect: "manual", credentials: "omit", signal: controller.signal,
      headers: { "User-Agent": "Alpha-Options-Research/0.2 (public economic release calendar)", Accept: "text/calendar" } }));
    if (response.status !== 200 || response.redirected) throw Error("HTTP_STATUS");
    if (!/^text\/calendar(?:\s*;\s*charset\s*=\s*(?:utf-8|"utf-8"))?\s*$/i.test(response.headers.get("content-type") ?? "")) throw Error("CONTENT_TYPE");
    const length = response.headers.get("content-length");
    if (length !== null && (!/^\d+$/.test(length) || Number(length) > RELEASE_CALENDAR_MAX_BYTES)) throw Error("BODY_TOO_LARGE");
    ...
    while (true) {
      ...
      if (bytes > RELEASE_CALENDAR_MAX_BYTES) throw Error("BODY_TOO_LARGE");
  ...
  if (sourceText !== null) {
    try { assessReleaseCalendar(input); }
    catch { input = { ...input, sourceText: null, errorCode: "SOURCE_SCHEMA" }; }
  }
  return { input, assessment: assessReleaseCalendar(input) };
}
...
export async function withReleaseCalendarJournal(workspace, operation, assessedAt = new Date().toISOString()) {
  releaseCalendarClock(assessedAt);
  const root = realpathSync(workspace);
  let directory = root;
  for (const part of ["data", "runtime", "options-release-calendar"]) {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (Object.keys(body).sort().join() !== "assessment,input,previousFingerprint,version" || body.version !== "1.0" ||
        body.previousFingerprint !== previousFingerprint || releaseCalendarFingerprint(body) !== fingerprint) fail("JOURNAL_INTEGRITY");
      const assessment = assessReleaseCalendar(body.input);
      if (releaseCalendarFingerprint(assessment) !== releaseCalendarFingerprint(body.assessment)) fail("JOURNAL_ASSESSMENT");
      inputs.push(body.input); previousFingerprint = fingerprint;
    }
    reportReleaseCalendar(inputs, assessedAt);
    const append = (input, savedAt = new Date().toISOString()) => {
      ...
      const next = structuredClone(input), assessment = assessReleaseCalendar(next);
      reportReleaseCalendar([...inputs, next], savedAt);
      if (inputs.length >= MAX_RETRIEVALS) fail("JOURNAL_LIMIT");
      const body = { version: "1.0", previousFingerprint, input: next, assessment };
      const fingerprint = releaseCalendarFingerprint(body), line = JSON.stringify({ ...body, fingerprint }) + "\n";
