...
await temp(async root=>{
  ...
  try{
    ...
    for(const path of ['/','/app.js','/i18n.js','/forms.js','/styles.css','/icon.svg'])await test('fixed asset served with CSP and no-store: '+path,async()=>{const r=await fetch(base+path);assert.equal(r.status,200);assert.equal(r.headers.get('cache-control'),'no-store');assert(r.headers.get('content-security-policy').includes("frame-ancestors 'none'"));assert((await r.text()).length>20);});
